...
func calculate(delta: float, motor: Motor, differential: Differential, gear: float) -> void:
	if input_value <= 0.0 or motor.rpm <= motor.idle_rpm:
		...
		clutch_locked = false
		return
	var clutch_max_torque := motor.max_torque * max_torque_factor * input_value * input_value
	...
	if not clutch_locked or clutch_locked and absf(motor.torque - axle_torque / gear) > clutch_max_torque:
		...
		var new_differential_av := axle_av + delta * differential_torque / differential_torque
