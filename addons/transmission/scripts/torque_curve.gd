...
@export_group("Points")
@export_custom(PROPERTY_HINT_NONE, "suffix:RPM") var idle_rpm := 800.0:
	set(value):
		idle_rpm = value
		emit_changed()


@export_custom(PROPERTY_HINT_NONE, "suffix:Nm") var idle_torque := 70.0:
	set(value):
		idle_torque = value
		emit_changed()


@export_custom(PROPERTY_HINT_NONE, "suffix:RPM") var effective_rpm := 1500.0:
	set(value):
		effective_rpm = value
		emit_changed()


@export_custom(PROPERTY_HINT_NONE, "suffix:Nm") var effective_torque := 120.0:
	set(value):
		effective_torque = value
		emit_changed()


@export_custom(PROPERTY_HINT_NONE, "suffix:RPM") var max_torque_rpm := 5000.0:
	set(value):
		max_torque_rpm = value
		emit_changed()


@export_custom(PROPERTY_HINT_NONE, "suffix:Nm") var max_torque := 151.0:
	set(value):
		max_torque = value
		emit_changed()


@export_custom(PROPERTY_HINT_NONE, "suffix:RPM") var max_power_rpm := 6300.0:
	set(value):
		max_power_rpm = value
		emit_changed()


@export_custom(PROPERTY_HINT_NONE, "suffix:HP") var max_power := 124.0:
	set(value):
		max_power = value
		emit_changed()


@export_custom(PROPERTY_HINT_NONE, "suffix:RPM") var max_rpm := 6500.0:
	set(value):
		max_rpm = value
		emit_changed()


@export_custom(PROPERTY_HINT_NONE, "suffix:Nm") var max_rpm_torque := 100.0:
	set(value):
